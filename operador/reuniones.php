...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['action']) && ($_POST['action'] === 'create' || $_POST['action'] === 'edit')) {
        ...
        if ($_POST['action'] === 'create') {
            ...
            log_action($_SESSION['user_id'], "Operador creó reunión: $tipo", "REUNIONES", $pdo->lastInsertId());
            ...
            log_action($_SESSION['user_id'], "Operador editó reunión ID: $id", "REUNIONES", $id);
    ...
    if (isset($_POST['action']) && $_POST['action'] === 'delete') {
        ...
        $stmt->execute([$id]);
        log_action($_SESSION['user_id'], "Operador eliminó reunión ID: $id", "REUNIONES", $id);
    ...
    if (isset($_POST['action']) && $_POST['action'] === 'finish') {
        ...
        $stmt->execute([$id]);
        log_action($_SESSION['user_id'], "Operador finalizó reunión ID: $id", "REUNIONES", $id);
            ...
            <tr>
                ...
                <td>
                    <div style="display: flex; gap: 0.3rem; flex-wrap: wrap;">
                        ...
                        <form method="POST" onsubmit="return confirm('¿Eliminar esta reunión?')" style="display:inline;">
                            <input type="hidden" name="csrf_token" value="<?php echo $csrf_token; ?>">
                            <input type="hidden" name="action" value="delete">
                            <input type="hidden" name="id_reunion" value="<?php echo $r['ID_REUNION']; ?>">
                            <button type="submit" class="btn btn-danger" style="font-size: 0.75rem;">Eliminar</button>
                        </form>
                        <a href="ficha_reunion.php?id=<?php echo $r['ID_REUNION']; ?>" class="btn btn-secondary" style="font-size: 0.75rem; background: #64748b; color: white;" target="_blank">PDF</a>
                    </div>
                </td>
            </tr>
            <?php endforeach; ?>
